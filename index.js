...
app.use(cors({credentials: true, origin: ["https://moto-gears-cf6ba.web.app", "http://localhost:3000"]}));
...
async function run (){
    try{
        ...
        const paymentCollection = client.db('moto_gears').collection('payments')
        ...
        app.get('/review', async (req, res) => {
         ...
         app.get('/parts/:id', async(req, res) => {
          ...
          app.put('/restockQty/:id', async(req, res) =>{
            const id = req.params.id;
            const qty = req.body.newQuantity;
            const filter = {_id: ObjectId(id)};
            const options = { upsert: true};
